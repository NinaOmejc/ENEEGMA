...
# Brain source: real component x, matching the canonical ENEEGMA node definition.
# -----------------------------------------------------------------------------

function f!(du, u, p, t)
    x, y = u
    r2 = x^2 + y^2
    du[1] = p.lambda * x - p.omega * y - r2 * x + p.I_ext(t)
    du[2] = p.omega * x + p.lambda * y - r2 * y
end
...
p = (lambda = 1.0, omega = 2pi * 10.0, I_ext = I_ext)
...
model_plot = plot_brain_source_results(brain_df; model_name="Stuart-Landau")
