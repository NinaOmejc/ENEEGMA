...
function _set_pop_from_grammar(parsed_rules::Vector{ParsedRule}, ip::Int, node::Node)::Population
    ...
    custom_cf_rule = ENEEGMA._get_parsed_rule_by_lhs(parsed_rules, "CF"; order=1)
    customCF = custom_cf_rule.rhs
    pop_rules = ENEEGMA._get_parsed_rule_by_lhs(parsed_rules, "Pop")
    ...
    pop_name = "P$(ip)"
    pop_rule = pop_rules[ip]
    pop_rule_pos_start = pop_rule.pos
    pop_rule_pos_end = ip < length(pop_rules) ? pop_rules[ip + 1].pos : custom_cf_rule.pos
